    ...
    try:
        ...
        if table == "nb_crm_contacts":
            record = await conn.fetchrow(
                ...
                mapped_data.get("source_system") or mapped_data.get("lead_source"),
                external_ids,
                entity_attrs,
                enrichment_status,
            )
        ...
        elif table == "nb_crm_leads":
            email = mapped_data.get("email")
            name = mapped_data.get("name") or mapped_data.get("label") or "Unknown"
            
            record = await conn.fetchrow(
                """
                INSERT INTO nb_crm_leads (
                    name, email, company, phone, title, description,
                    source, status, created_at, updated_at
                ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())
                ON CONFLICT (email)
                DO UPDATE SET
                    name = COALESCE(EXCLUDED.name, nb_crm_leads.name),
                    company = COALESCE(EXCLUDED.company, nb_crm_leads.company),
                    phone = COALESCE(EXCLUDED.phone, nb_crm_leads.phone),
                    title = COALESCE(EXCLUDED.title, nb_crm_leads.title),
                    description = COALESCE(EXCLUDED.description, nb_crm_leads.description),
                    source = COALESCE(EXCLUDED.source, nb_crm_leads.source),
                    updated_at = NOW()
                RETURNING id, created_at
                """,
                name,
                email,
                mapped_data.get("company"),
                mapped_data.get("phone"),
                mapped_data.get("title"),
                mapped_data.get("description"),
                mapped_data.get("source"),
                mapped_data.get("status", "new"),
            )
            return {"id": record["id"], "created": record["created_at"]}

        elif table == "nb_crm_customers":
            label = mapped_data.get("label") or mapped_data.get("name") or "Unknown"
            
            record = await conn.fetchrow(
                """
                INSERT INTO nb_crm_customers (
                    name, phone, description, status, created_at, updated_at
                ) VALUES ($1, $2, $3, $4, NOW(), NOW())
                ON CONFLICT (name)
                DO UPDATE SET
                    phone = COALESCE(EXCLUDED.phone, nb_crm_customers.phone),
                    description = COALESCE(EXCLUDED.description, nb_crm_customers.description),
                    updated_at = NOW()
                RETURNING id, created_at
                """,
                label,
                mapped_data.get("phone"),
                entity_attrs.get("description") if entity_attrs else None,
                mapped_data.get("status", "active"),
            )
            return {"id": record["id"], "created": record["created_at"]}
