  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto text-center">
        ...
        <h2 className="text-[clamp(32px,5vw,56px)] font-black leading-[1.1] tracking-[-0.02em] text-white mb-16">
          {t('headline')}
        </h2>

        <div className="flex flex-wrap justify-center gap-8 md:gap-16 py-10 border-t border-b border-white/[0.08] mb-12">
          {stats.map((stat) => (
            <div key={stat.label} className="bl-stat__item">
        ...
        <div className="flex flex-wrap justify-center gap-10">
          {trustItems.map((item) => (
            <span
              key={item}
              className="text-[11px] font-bold tracking-[0.2em] uppercase text-white/35"
            >
              {item}
            </span>
          ))}
